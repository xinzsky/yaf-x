 ...
 * 
 * 注意：此类依赖mysql扩展模块，该扩展自 PHP 5.5.0 起已废弃，并在将来会被移除。
 *      应修改使用 MySQLi 或 PDO_MySQL 扩展来替换之。
 */

class Gek_Utils_Mptt
{
                                ...
                                $parent_column = 'parent') 
    {
        ...
        if(isset($dsn['host'])) {$host = $dsn['host'];}
        if(isset($dsn['port'])) {$port = $dsn['port'];}
        if(isset($dsn['user'])) {$user = $dsn['user'];}
        if(isset($dsn['password'])) {$password = $dsn['password'];}
        if(isset($dsn['db'])) {$db = $dsn['db'];}

        // continue only if there is an active MySQL connection
         if (!($connection = @mysql_connect($host.':'.$port, $user, $password))) {
                return;
         }

        @mysql_select_db($db, $connection);
        
        @mysql_query('SET NAMES utf8'); //added by xinz
